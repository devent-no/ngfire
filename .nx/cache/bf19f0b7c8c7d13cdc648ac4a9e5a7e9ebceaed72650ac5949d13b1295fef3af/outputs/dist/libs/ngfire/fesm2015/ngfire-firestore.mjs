...
import { InjectionToken, inject, InjectFlags, Injector, PLATFORM_ID, Injectable, NgZone } from '@angular/core';
...
const FIRESTORE = new InjectionToken('Firestore instance', {
    ...
    factory: () => {
        ...
        if (config.firestore) {
            return config.firestore(app, settings !== null && settings !== void 0 ? settings : {});
        }
        else {
            return initializeFirestore(app, settings !== null && settings !== void 0 ? settings : {});
        }
    },
});
...
class FireDocument {
    ...
    useCache(ref) {
        if (isPlatformServer(this.platformId)) {
            return this.zone.runOutsideAngular(() => fromRef(ref)).pipe(switchMap((snap) => __awaiter(this, void 0, void 0, function* () { return this.snapToData(snap); })), tap(value => this.firestore.setTransfer(ref, value)), keepUnstableUntilFirst(this.zone));
        }
        if (!this.memorize) {
            return this.zone.runOutsideAngular(() => fromRef(ref)).pipe(switchMap((snap) => __awaiter(this, void 0, void 0, function* () { return this.snapToData(snap); })), keepUnstableUntilFirst(this.zone));
        }
        ...
        if (transfer)
            return snap$.pipe(switchMap((snap) => __awaiter(this, void 0, void 0, function* () { return this.snapToData(snap); })), startWith(transfer));
        if (initial)
            return snap$.pipe(startWith(initial), switchMap((snap) => __awaiter(this, void 0, void 0, function* () { return this.snapToData(snap); })));
        return snap$.pipe(switchMap((snap) => __awaiter(this, void 0, void 0, function* () { return this.snapToData(snap); })));
    }
    clearCache(ref) {
    ...
    toFirestore(entity, actionType) {
        if (actionType === 'create') {
            ...
            return Object.assign({ _meta }, entity);
        }
        else {
            return Object.assign(Object.assign({}, entity), { '_meta.modifiedAt': new Date() });
        }
    }
    /** Function triggered when getting data from firestore */
    fromFirestore(snapshot) {
        if (snapshot.exists()) {
            return Object.assign(Object.assign({}, toDate(snapshot.data())), { [this.idKey]: snapshot.id });
        }
        else {
            return undefined;
        }
    ...
    createId(params) {
        var _a;
        return (_a = this.getRef(params)) === null || _a === void 0 ? void 0 : _a.id;
    }
    ...
    upsert(document, options = {}) {
        ...
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const snap = ((options === null || options === void 0 ? void 0 : options.write) instanceof Transaction)
                ? yield ((_a = options.write) === null || _a === void 0 ? void 0 : _a.get(ref))
                : yield getDoc(ref);
            if (snap.exists())
                return this.create(document, options);
            yield this.update(document, options);
            return id;
        });
    }
    /**
    ...
    create(document, options = {}) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const data = yield this.toFirestore(document, 'create');
            if (this.storeId)
                data[this.idKey] = id;
            const ref = this.getRef(options.params);
            if (options.write) {
                ...
                yield setDoc(ref, data);
            }
            if (this.onCreate) {
                yield this.onCreate(data, { write: options.write, ctx: options.ctx });
            }
            return id;
        });
    }
    ...
    delete(options = {}) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            write.delete(ref);
            if (this.onDelete) {
                yield this.onDelete(ref.path, { write, ctx });
            }
        ...
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (typeof changes === 'function') {
                yield runTransaction(this.db, (tx) => __awaiter(this, void 0, void 0, function* () {
                    ...
                    const doc = yield this.fromFirestore(snapshot);
                    if (doc && changes) {
                        const data = yield changes(doc, tx);
                        const result = yield this.toFirestore(data, 'update');
                ...
                const doc = yield this.toFirestore(changes, 'update');
                if (options.write) {
                    options.write.update(ref, doc);
                }
                else {
                    yield updateDoc(ref, doc);
                }
                if (this.onUpdate) {
                    yield this.onUpdate(doc, { write: options.write, ctx: options.ctx });
                }
            }
            if (this.memorize)
                this.clearCache(ref);
...
class FireCollection {
    ...
    clearCache(refs) {
        ...
        return this.firestore.clearCache(refs === null || refs === void 0 ? void 0 : refs.path);
    }
    /** Function triggered when adding/updating data to firestore */
    toFirestore(entity, actionType) {
        if (actionType === 'add') {
    ...
    getFromRef(ref) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const snap = (ref.type === 'document') ? yield getDoc(ref) : yield getDocs(ref);
            return this.snapToData(snap);
        });
    }
    fromRef(ref) {
    ...
    upsert(documents, options = {}) {
        return __awaiter(this, void 0, void 0, function* () {
            const doesExist = (doc) => __awaiter(this, void 0, void 0, function* () {
                var _a;
            ...
            const upsert = (doc) => __awaiter(this, void 0, void 0, function* () {
                const exists = yield doesExist(doc);
                if (!exists)
                    return this.add(doc, options);
                yield this.update(doc, options);
    ...
    add(documents, options = {}) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const operations = docs.map((value) => __awaiter(this, void 0, void 0, function* () {
                const id = value[this.idKey] || this.createId();
                const data = yield this.toFirestore(value, 'add');
                if (this.storeId)
                    data[this.idKey] = id;
                const ref = this.getRef(id, options.params);
                write.set(ref, data);
                if (this.onCreate) {
                    yield this.onCreate(data, { write, ctx });
                }
                return id;
            }));
            const ids = yield Promise.all(operations);
            // If there is no atomic write provided
            if (!options.write) {
                yield write.commit();
            }
    ...
    remove(id, options = {}) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const refs = [];
            const operations = ids.map((docId) => __awaiter(this, void 0, void 0, function* () {
                const ref = this.getRef(docId, options.params);
                write.delete(ref);
                if (this.onDelete) {
                    yield this.onDelete(docId, { write, ctx });
                }
                refs.push(ref);
            }));
    ...
    removeAll(options = {}) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            yield this.remove(ids, options);
            if (this.memorize)
                this.clearCache(ref);
        });
    }
    update(idsOrEntity, stateFnOrWrite, options = {}) {
        return __awaiter(this, void 0, void 0, function* () {
            let ids = [];
            let stateFunction;
            let getData;
            ...
            if (stateFunction) {
                ...
                yield runTransaction(this.db, (tx) => __awaiter(this, void 0, void 0, function* () {
                    refs = [];
                    const operations = ids.map((id) => __awaiter(this, void 0, void 0, function* () {
                        const ref = this.getRef(id, options.params);
                        refs.push(ref);
                        const snapshot = yield tx.get(ref);
                        const doc = this.fromFirestore(snapshot);
                        if (doc && stateFunction) {
                            const data = yield stateFunction(doc, tx);
                            const result = yield this.toFirestore(data, 'update');
                            tx.update(ref, result);
                            if (this.onUpdate) {
                                yield this.onUpdate(data, { write: tx, ctx });
                            }
                        }
                        return tx;
                    }));
                ...
                const operations = ids.map((docId) => __awaiter(this, void 0, void 0, function* () {
                    ...
                    const ref = this.getRef(docId, options.params);
                    refs.push(ref);
                    const data = yield this.toFirestore(doc, 'update');
                    write.update(ref, data);
                    if (this.onUpdate) {
                        yield this.onUpdate(doc, { write, ctx });
                    }
                }));
                yield Promise.all(operations);
                // If there is no atomic write provided
...
class FireSubCollection extends FireCollection {
    ...
    fromFirestore(snapshot) {
        if (snapshot.exists()) {
            return Object.assign(Object.assign({}, toDate(snapshot.data())), { [this.idKey]: snapshot.id, [this.pathKey]: snapshot.ref.path });
        }
        else {
            return undefined;
        }
